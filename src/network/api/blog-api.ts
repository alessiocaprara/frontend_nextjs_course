...
export async function createBlogPost(input: CreateBlogPostValues) {
    ...
    const response = await api.post<BlogPost>("/posts", formaData);
    return response.data;
}
...
export async function updateBlogPost(blogPostId: string, input: UpdateBlogPostValues) {
    const formaData = new FormData();
    Object.entries(input).forEach(([key, value]) => {
        if (value !== undefined) formaData.append(key, value);
    });
    await api.patch("/posts/" + blogPostId, formaData);
}
...
export async function getCommentsForPost(postId: string, continueAfterId?: string) {
    const response = await api.get<CommentsPage>(`/posts/${postId}/comments?${continueAfterId ? "continueAfterId=" + continueAfterId : ""}`);
    return response.data;
}
...
export async function createComment(postId: string, parentCommentId: string | undefined, text: string) {
    const response = await api.post<Comment>(`/posts/${postId}/comments`, { text, parentCommentId });
    return response.data;
}
