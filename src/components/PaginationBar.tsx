...
const PaginationBar = ({ pageCount, currentPage, onPageItemClicked, className }: PaginationBarProps) => {
    ...
    for (let i = paginationMinPage; i <= paginationMaxPage; i++) {
        ...
        if (i === currentPage) {
            ...
            paginationItem =
                <Pagination.Item key={i} className="d-none d-md-block" onClick={() => onPageItemClicked(i)}>
                    {i}
                </Pagination.Item>
        }
